...
pub(crate) struct GradClipper;

impl GradClipper {
    pub(crate) fn clip_grad_norm(params: &[&Tensor], max_norm: f32) -> f32 {
    ...
    pub(crate) fn clip_grad_value(params: &[&Tensor], clip_value: f32) {
...
mod tests {
    ...
    /// @covers: GradClipper::clip_grad_norm
    #[test]
    fn test_grad_clipper_struct_clip_grad_norm_returns_zero_when_no_grads() {
        tape::clear_tape();
        let t = Tensor::zeros(vec![3]);
        let norm = GradClipper::clip_grad_norm(&[&t], 1.0);
        assert!(norm.abs() < 1e-6);
    }

    /// @covers: GradClipper::clip_grad_value
    #[test]
    fn test_grad_clipper_struct_clip_grad_value_does_not_panic_without_grads() {
        tape::clear_tape();
        let t = Tensor::zeros(vec![3]);
        GradClipper::clip_grad_value(&[&t], 0.5);
    }
}
